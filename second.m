...
Z = get_song_wave_2(ADSR_song, beat_length, fs);

% Apply the ADSR envelope to the waveform
Y = ADSR_env(Z, beat_length, 0.25, fs); 

% Play the processed song
soundsc(Y, fs); 

audiowrite('second.wav', Z, fs);
...
% Calculate the spectrogram with no overlap
noverlap = 0;

[S, F, T, P] = spectrogram(Z, window_length, noverlap, window_length, fs);
...
colorbar;
caxis([-80 -30]); % Adjust the color axis for better visibility if needed
